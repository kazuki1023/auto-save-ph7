...
const eventDetailsData: EventDetails = {
  title: '新入生歓迎会',
  description:
    '新入生のみなさん、ぜひご参加ください！軽食とドリンクを用意します🎉',
  category: '新歓イベント',
  capacity: 100,
  location: '大学講堂',
  responseRate: 89,
    ...
    <div className="bg-violet-50 min-h-screen font-sans">
      <div className="max-w-2xl mx-auto p-4 space-y-6">
        ...
        <ParticipationSummaryCard
          ...
          capacity={eventDetailsData.capacity}
        />
  ...
  <header className="space-y-4">
    ...
    <div className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm text-slate-600">
      ...
      <InfoItem
        ...
        text={`定員${details.capacity}人`}
      />
      <InfoItem
        icon={<PieChart className="w-4 h-4 text-slate-400" />}
        text={`回答率${details.responseRate}%`}
      />
    </div>
  </header>
);
    ...
    <div className="flex justify-between items-start">
      <div>
        ...
        <p className="mt-2 text-sm text-slate-600">
          予想参加者: {info.expectedParticipants}人
        </p>
      </div>
...
const ParticipationSummaryCard: React.FC<{
  summaryItems: SummaryItem[];
  capacity: number;
}> = ({ summaryItems, capacity }) => (
...
const SummaryItem: React.FC<{ item: SummaryItem; capacity: number }> = ({
  item,
  capacity,
}) => {
  const rankColors = [
    ...
    <div>
      <div className="flex justify-between items-start">
        ...
        <div className="text-right">
          ...
          <span
            className={`text-xs font-semibold px-2 py-0.5 rounded-full ${statusColors[item.status]}`}
          >
            {item.status}
          </span>
        </div>
      ...
      <div className="mt-4 pl-10 space-y-3 text-sm">
        <ProgressBar
          label="参加"
          count={item.attending}
          total={capacity}
          icon={<CheckCircle2 className="w-4 h-4 text-green-500" />}
        />
        <ProgressBar
          label="検討中"
          count={item.considering}
          total={capacity}
...
const NonParticipationAnalysisCard: React.FC<{
  reasons: NonParticipationReason[];
}> = ({ reasons }) => (
